...
if __name__ == '__main__':
    params = {
        ...
        'relax': 5e-9,  # Параметр релаксации (влияет на амплитуду шума в силах)
        'dt': 1e-13,  # Шаг по времени
        'steps': 120000,  # Общее количество шагов
    ...
    print(f'Средняя температура: {np.mean(output[:, 1])} (K)')
    ...
    print(f'Среднее давление: {np.mean(pressures)} (Па)')
